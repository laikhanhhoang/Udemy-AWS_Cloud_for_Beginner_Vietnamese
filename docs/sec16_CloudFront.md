...
- [Lí thuyết](#lí-thuyết)
    - [CDN - Định nghĩa](#cdn---định-nghĩa): **giúp delivery nội dung nhanh chóng nhờ cache tài nguyên** trên máy chủ gần.
    - [Cloud Front - Khái niệm cơ bản](#cloud-front---khái-niệm-cơ-bản): cache, loggin, security.
    - [Cách hoạt động của Cloud Front](#cách-hoạt-động-của-cloud-front)
    - [Usecase](#usecase)
    - [Pricing](#pricing)
    - [CloudFront behavior](#cloudfront-behavior): **điều hướng request tới đúng origin mong muốn** như **`/*`, `api/*`**,...
    - [CloudFront Cache Policy](#cloudfront-cache-policy): **tùy chỉnh cách cache và serves content**: có thể cache hoặc không đối với API,...
    - [CloudFront Origin request policy](#cloudfront-cache-policy): định nghĩa cách CloudFront **xử lý request tới origin** như S3, EC2, ALB,...
- [Lab](#lab)
