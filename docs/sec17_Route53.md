...
- [Lí thuyết](#lí-thuyết)
    - [Route 53 - Định nghĩa](#route-53---định-nghĩa): **quản lý các tên miền và ánh xạ chúng**.
    - [3 tính năng chính](#3-tính-năng-chính)
    - [DNS Resolver](#dns-resolver)
    - [Route 53 Hosted Zone](#route-53---định-nghĩa)
        - Các loại DNS Record: **`A (domain → IP), CNAME (domain → domain)`**,...
    - [Routing policy](#routing-policy): Simple, Weighted, Latency,...
    - [SSL Certificate](#ssl-certificate): Do ACM cấp cho một **domain hoặc subdomain duy nhất** để **xác minh quyền sở hữu domain**.
    - [Pricing](#pricing)
    - [Lưu ý](#lưu-ý)
- [Lab](#lab)
...
Hình dưới mô tả một flow từ khi người dùng gõ địa chỉ của một trang web **`www.example.com`** lên trình duyệt cho tới khi nội dung thực sự được trả về từ máy chủ.
...
Do ACM cấp cho một **domain hoặc subdomain duy nhất** , **bao gồm CNAME Name và CNAME Value** để **xác minh quyền sở hữu domain**.
- Bạn phải **tạo record CNAME trong hosted zone với Name = CNAME Name và Value = CNAME Value**.
- Sau khi ACM xác minh, certificate được issue và có thể dùng cho các dịch vụ AWS để bật HTTPS.
- Một số trường hợp phổ biến:
    - Trỏ domain **từ Route 53 tới CloudFront** và **bật HTTPS**.
    - **Cho phép client gửi HTTPS request tới các dịch vụ AWS** như ALB / API Gateway / ELB.


####  **Health Check**
- Route 53 định kỳ thực hiện các call tới endpoint bạn muốn 
thực hiện healthcheck. Nếu response failed hoặc không trả 
về response, Route 53 sẽ raise alarm tới CloudWatch 
Alarm kết hợp với SNS để notify tới người nhận (vd đội 
operator).
...
- Chi tiết hơn tham khảo: https://aws.amazon.com/route53/pricing/.

####  **Lưu ý**
- Nếu muốn trỏ domain ta mua thẳng tới IP (Elastic) của EC2 thì ta phải tạo 2 record, một record giả và một record bắc cầu, cụ thể:
    - Record 1: **`hosted_record.myweb.click` → A → EC2 Elastic IP**.
    - Record 2: **`myweb.click` → A (Route to AWS Resource Type) → `hosted_record.myweb.click`**.
